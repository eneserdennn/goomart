...
const CampaignCard: FC<CampaignCardProps> = ({ campaign, isSelectable = true }) => {
    ...
    return (
        <div className="flex flex-row bg-white px-[10px] py-[25px] justify-between h-[116px] items-center rounded-md">
            ...
            <div className="flex flex-col pl-2 flex-grow"> {/* Allow the text to take up remaining space */}
                <span className={`font-bold text-[13px] mb-2 text-[#363636] ${isSelectable && 'max-w-[164px]'}`}>
                    {name}
                </span>
                {isSelectable && (<div className="flex flex-row">
                    {isActive && (
                        <>
