...
type ProductType = {
  id: string | number; // You can adjust this based on your actual type
  name: string;
...
const SideBar = ({ data }) => {
  ...
  const [tempSelectedProductTypeList, setTempSelectedProductTypeList] =
    useState([]);
  ...
  const CustomCheckbox = ({ label, isChecked, onCheckChange }: any) => {
    ...
    return (
      <div
        className="flex bg-white flex-row items-center border-b h-[52px] px-[15px]"
          ...
          <div className="flex items-center">
            <div className="flex flex-col">
              {checked ? (
                ...
                <div className="flex items-center justify-center border-2 border-[#969696] rounded-md h-[20px] w-[20px]"></div>
              )}
            </div>
          </div>
  ...
  const CustomRadio = ({ label, value }: any) => {
    return (
      <div
        className="flex bg-white flex-row items-center border-b h-[52px] px-[15px]"
        onClick={() => {
  ...
  return (
    <>
      <Image
        src={ICONS.filter}
        alt={"filter"}
        className="h-8 w-8 bg-white p-1.5 rounded-md"
      ...
      >
        ...
        >
          {" "}
          {/* Buradaki 150px değeri header ve footer'ın toplam yüksekliği olarak varsayıldı. */}
          {selectedPage === pages[0] && (
            <>
              <div
                className="flex flex-col justify-center white h-[53px]"
                onClick={() => {
          ...
          {selectedPage === pages[3] && (
            <>
              {allProductTypes.map((productType: ProductType) => (
                <CustomCheckbox
                  key={productType.id}
        ...
        {selectedPage === pages[0] && (
          <div className="fixed bottom-0 left-0 w-full bg-white ">
            <div className="flex justify-center py-2">
              ...
              >
                Ürünleri Görüntüle{" "}
                {filtered && <span>({filteredProductCount})</span>}
              </button>
            </div>
          </div>
        )}
      ...
      {toggleSideBar && (
        <div
          ...
          className="fixed top-0 right-0 bg-black/[.54] w-full min-h-screen z-40"
        ></div>
      )}
    </>
  );
