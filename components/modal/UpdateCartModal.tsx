...
const UpdateCartModal: React.FC<UpdateCartModalProps> = ({
    ...
    return show ? (
        <div className="fixed inset-0 flex items-center justify-center z-50">
            ...
            ></div>
            <div
                className="bg-white rounded-2xl w-[338px] h-[663px] z-10 flex flex-col items-center pb-[21px] relative">
                <div className="bg-white  rounded-full p-3 pb-[25px] -mt-[36px] flex flex-col">
                    {icon ? icon : <Image src={ICONS.warning} alt={"warning"} className="h-[56px] w-[56px] mb-[15px]"/>}

                </div>
                <div className="flex flex-col w-full items-center px-[15px]">
                <span className="text-center text-[#333] font-bold text-[15px]">Sepetiniz güncellenmistir</span>
                <span className="text-center text-[13px] font-bold mt-[10px] mb-[45px]">
                    <span className="text-rose-600">Stok tükendi.</span> Sepetinizde bulunan bazi ürünler stokta tükenmesi nedeniyle sepetinizden kaldirildi.
                </span>

                <div className="flex flex-row w-full items-center justify-between px-[5px] pb-[10px]">
                    <div className="flex items-center">
                    <div className="flex w-[70px] h-[70px] bg-[#B8B8B8] flex-shrink-0">
                    </div>
                ...
                <div className="flex flex-col bg-white border w-full items-center bottom-0 px-[15px] py-[19px] absolute">
                    <Button
                        className={`text-white ${!hasCancelButton && 'w-full'} w-full bg-primary hover:bg-green-800 font-semibold rounded-lg shadow-md hover:shadow-lg transition duration-300`}
                        onClick={onConfirm}
                        isSmall={true}
                    >
                        Tamam
