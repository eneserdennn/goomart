...
import { useGetAllCampaignsQuery, useGetCampaignsUsableQuery } from "@/redux/features/campaigns/campaignApiSlice";
...
import {
    setShowModal, setInputValue, setAppliedCoupon, setWrongCoupon,
    selectCampaigns
...
const CampaignsApply: FC = () => {
    ...
    const wrongCoupon = useSelector(state => selectCampaigns(state).wrongCoupon);
    ...
    const handleConfirm = () => {
        const matchedCoupons = coupons?.filter(coupon => coupon.code === inputValue);
    ...
    const content = (
        <>
                    ...
                    >
                        <div className="flex flex-row px-[15px] items-center w-full">
                            {appliedCoupon ? (
                                ...
                                <div onClick={() => dispatch(setShowModal(true))} className="flex flex-row items-center">
            ...
            <InputModal
                ...
                buttonText={"Tamam"}
                icon={
                    <Image src={ICONS.couponCode} alt={"warning"} className="h-[56px] w-[56px]"/>
                }
            />
            <Modal show={wrongCoupon} onClose={() => dispatch(setWrongCoupon(false))}
                   message={"Kupon Kodu Geçersiz"} hasCancelButton={false}
                   buttonText={"Tamam"}
                   onConfirm={() => {
                       dispatch(setWrongCoupon(false));
                   }}
            />
            <Modal show={deleteCouponModal} onClose={() => setDeleteCouponModal(false)}
                   message={"Kupon kodu kaldiriacaktir, devam etmek istediginize emin misiniz?"}
                   buttonText={"Tamam"}
                   onConfirm={() => {
