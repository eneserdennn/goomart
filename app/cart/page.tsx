    ...
    <div>
      {data && data.products.length > 0 ? (
        data.products.map((item) => {
          ...
          if (item.productUnitItself.stock === 0) {
            content = <div>Urunun stoku bitmis</div>;
          } else if (item.productUnitItself.stock > 0) {
            content = <div>Urunun stogu azaliyor</div>;
          }
          return content;
        })
      ) : (
...
//       {data?.products.map(
//         (item: Product) => (
//           console.log(item), (<CartProduct product={item} key={item.id} />)
//         )
//       )}
//       {isLoading ? (
//         <Loading />
