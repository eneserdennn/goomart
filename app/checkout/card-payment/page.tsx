...
const IconInput: React.FC<IconInputProps> = ({icon: Icon, mask, ...props}) => {
                ...
                >
                    {(inputProps) => (
                        <input
                            {...props}
                            {...inputProps}
                            className={`appearance-none border ${errorText ? 'border-red-500' : 'border-0'} p-[15px] h-[53px] rounded-lg w-full py-2 px-4 text-gray-700 leading-tight focus:outline-none focus:shadow-outline pr-10`} // px değerini sağa aldık (pr-10)
                        />
                    )}
                </InputMask>
...
const CardValidationSchema = Yup.object().shape({
    ...
    monthAndYear: Yup.string()
        ...
        .test('expiration-date', 'Son kullanma tarihi geçmiş bir tarihe ait olamaz.', (value) => {
    ...
    return (
        <div className="flex flex-col w-full py-[20px]">
            ...
            >
                {formik => (
                    <Form className="flex flex-col w-full px-[15px]">
                        ...
                        <button type="submit" className="bg-primary text-white h-[53px] rounded-tl-3xl rounded-br-3xl">
                            58,94 € Öde
                        </button>
                    </Form>
                )}
            ...
            <div className="flex flex-row h-[53px] bg-white mt-[40px] items-center justify-between px-[105px]">
                ...
                <Image src={ICONS.mastercard} alt={'mastercard'} width={36} height={24} />
                <Image src={ICONS.americanexp} alt={'americanexp'} width={36} height={24} />
