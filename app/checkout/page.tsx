...
const CheckoutPage = () => {
    ...
    const cargoMethods = [
        {title: "DHL Standart EU", desc: "Tahmini teslimat süresi 1-2 gün", image: IMAGES.paymentMethod}
    ]

    const paymentMethods = [
        {title: "Kredi Kartı", imageArray: [ICONS.visa, ICONS.mastercard, ICONS.americanexp, ICONS.discover]},
            ...
            <div className="flex items-center justify-between bg-white w-full border h-[70px] rounded p-4" onClick={
                ...
                <div className="flex">
                    <div className="flex items-center">
                        {selectedCargoMethod === title || selectedPaymentMethod === title ? (
                            <IoMdRadioButtonOn className="text-primary mr-4 h-6 w-6"/>
                        ) : (
                            <IoMdRadioButtonOff className="text-gray-300 mr-4 h-6 w-6"/>
                        )}
                    </div>
                    <div className="flex items-center"> {/* Address section takes 3/5 of the width */}
                        <div className="flex flex-col space-y-1">
                            <span className="text-[13px] font-bold">
                                {title}
                            </span>
                            <span
                                className="text-[13px] font-semibold text-[#6D6D6D]">
                                {desc}
                            </span>
                        </div>
                    </div>
                        ...
                        imageArray && (
                            <div className="flex">
                                {imageArray.map((image, idx) => (
                                    <Image key={idx} src={image} alt={"cargo-method"} width={23} height={15}
                                           className="mr-1"/>
                                ))}
                            </div>
                        )
        ...
        <div className="flex flex-col">
            ...
            <div className="flex flex-col ">
                {cargoMethods.map((cargoMethod, idx) => (
                    <CustomRadio key={idx} title={cargoMethod.title} desc={cargoMethod.desc} image={cargoMethod.image}
            ...
            <div className="flex flex-col ">
                {paymentMethods.map((paymentMethod, idx) => (
                    <CustomRadio key={idx} title={paymentMethod.title} image={paymentMethod.image}
            ...
            <Link href={`/campaigns/apply`}
                  className="flex flex-row pl-[10px] border pr-[18px] h-[60px] bg-white justify-between items-center">
                <div className="flex flex-row items-center ">
                    {!appliedCoupon ? (<><Image src={ICONS.campaignCheckout} alt={"campaign"} width={48} height={48}
                                                className="pr-[15px]"/>
                        <span className="text-[14px] font-bold">Kampanya Sec</span></>) : (
                        <div className="flex flex-row">
                            <div className="flex items-center">
                                <Image src={ICONS.couponCodeNoBg} alt={"campaign"} width={40} height={40}
                                       className="pr-[15px]"/>
            ...
            <div className="flex fixed justify-center flex-row bottom-[70px] bg-white left-0 w-full">
                ...
                <div className="flex w-[87px] mx-[34px] mt-1 items-center justify-center line-through">
                    <span className="text-[#AAAAAA] text-[16px] font-bold items-center">123123</span>
                    <span className="text-[#AAAAAA] text-[16px] font-bold ml-1 items-center">€</span>
                </div>
                    <div className="flex w-[87px] mx-[34px] items-center justify-center ">
                        <span className="text-primary text-[21px] font-bold items-center">123123</span>
